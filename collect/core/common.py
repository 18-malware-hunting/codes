...
def retrieve_content(url, data=None, headers=None):
    ...
    try:
        req = _urllib.request.Request("".join(url[i].replace(' ', "%20") if i > url.find('?') else url[i] for i in range(len(url))), data, headers or {"User-agent": "NAME", "Accept-encoding": "gzip, deflate"})
        resp = _urllib.request.urlopen(req, timeout=100)
        retval = resp.read()
    ...
    if six.PY3 and isinstance(retval, bytes):
        retval = retval.decode("utf8", errors="replace")
